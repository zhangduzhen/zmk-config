...
#+caption: 默认层
[[file:images_latest/default_layer.png]]

#+caption: windows层
[[file:images_latest/windows_layer.png]]
...
#+caption: right layer
[[file:images_latest/right_layer.png]]
...
#+caption: left layer
[[file:images_latest/left_layer.png]]
...
-- 这层修改数字和字母键位都没问题，但是刷到~出现了左右手问题，也可能是硬件电量和蓝牙连接问题。换成左手与电脑有线连接后，从左手bootloader进刷完暂时解决。原因之后再说。
- 按住左tab (来进入left layer), 然后按空格键进入mouse layer
- 按 =p= 或 =q= 退出鼠标层
- =MB4= 对应鼠标的后退键, =MB5= 对应鼠标的前进键

#+caption: mouse layer
[[file:images_latest/mouse.png]]
